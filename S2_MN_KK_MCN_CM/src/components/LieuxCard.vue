...
const props: LieuxResponse = defineProps<LieuxResponse>()
...
<template>
      <RouterLink :to="{ name: 'lieux-id', params: { id } }" class="lg:col-span-3">  
        <div
            class="shadow-2xl flex flex-col justify-start  overflow-hidden gap-[3px] rounded-tl-[51.94px]  rounded-br-[51.94px] bg-card mb-14">
            <img :src="urlImg0" class="self-stretch  flex-grow-0 h-52 flex-shrink-0  rounded-tl-[51.94px]  object-cover" />
         ...
         <p class="h3 text-left font-medium text-text">
                        {{ region }}
            </p>
                </div>
            </div>
